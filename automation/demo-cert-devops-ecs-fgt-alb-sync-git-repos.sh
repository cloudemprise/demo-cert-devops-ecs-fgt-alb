...
REPO_DESCRIPTION=\
"This repository contains artifacts and reference material related to an AWS \
ECS sample project used to perform a deployment of a docker web application to \
a AWS Fargate ECS Container that comprises an Application Load Balancer. It \
just provides a basic infrastructure skeleton in which to investigate the AWS \
ECS Service in more detail."
while true
do
  # -e : stdin from terminal
  # -r : backslash not an escape character
